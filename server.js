...
if (config.server.update) {
    requester(config.server.link, (err, req, data) => {
        ...
        if (config.server.version < requesterConfig.server.version) {
            logger.warn(`new update was found`)
            logger.warn(`Download -> https://agarbots4u.wixsite.com/home`)
			process.exit();
        } else {
            logger.good(`No updates found`)
        }
    })
} else {
    logger.error('Update is false')
}
...
const dataBot = {
    ...
    onclose() {
        if (this.isConnected) {
            this.isConnected = false
            this.connect()
            logger.error('Data disconnected!')
        }
    },
    handleBuffer(buffer) {
        ...
        switch (reader.readUint8()) {
            case 0:
                if (!user.startedBots) {
                    ...
                    logger.good('Starting')
                    logger.good("tokens:", user.tokens.length);
                }
                break
            case 1:
                if (user.startedBots && !stoppingBots) {
                    ...
                    setInterval(() => {
                        if (seconds === 30) {
                            ...
                            logger.warn(`Stopping bots ${30 - seconds} seconds`)
                            seconds++
                        }
                    }, 1000)
                }
